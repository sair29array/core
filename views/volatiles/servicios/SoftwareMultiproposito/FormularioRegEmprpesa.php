...
<?php if (!isset($_GET['solicitud-enviada'])) {
 ...
 <?php 
 	if (isset($_POST["continuar_contrato2"])) {
 		...
 		// añanir emppresa a user desde la pagina de plan basico del servicio de Diseño  De Pagina web
 		$sair->add_empresa_to_user_pb_dw($ne,$nie,$id_user);
 		...
 		$id_plan = 1 ; // quiere decir que es el plan numero 1 dentro de la db osea el basico de escritorio 
 		if ($ne == "" && $nie == "") {
        ...
        ///////////////////////
 		...
 		?> <script type="text/javascript">window.location="../[array]/?servicios=software_multiproposito&solicitud=plan-escritorio&Reg-empresa&solicitud-enviada"</script> <?php 
...
// si esto pasa es por que ya el user tiene una empresa registrada :
   ...
   //$fecha = date("d")." de ". date("m"). " de ". date("y"); 
        ...
        ///////////////////////
 		...
 		$sair->registrar_solicitud_contrato($id_user,"software_multiproposito",$id_plan,$facturacion_empresa);
 		?> <script type="text/javascript">window.location="../[array]/?servicios=Sofware-Multipropósito&solicitud=plan-escritorio&Reg-empresa&solicitud-enviada"</script> <?php 
 	}
